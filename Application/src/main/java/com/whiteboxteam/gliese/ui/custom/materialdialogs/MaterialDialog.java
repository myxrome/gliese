...
import com.whiteboxteam.gliese.ui.custom.materialdialogs.base.DialogBase;
import com.whiteboxteam.gliese.ui.custom.materialdialogs.util.DialogUtils;
import com.whiteboxteam.gliese.ui.custom.materialdialogs.util.RecyclerUtil;
import com.whiteboxteam.gliese.ui.custom.materialdialogs.util.TypefaceHelper;
...
public class MaterialDialog extends DialogBase implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    protected final void updateFramePadding() {
        ...
        if (contentScrollView != null) {
            ...
            contentScrollView
                    .setPadding(contentScrollView.getPaddingLeft(), paddingTop, contentScrollView.getPaddingRight(),
                                paddingBottom);
        }

        if (listView != null) {
            // Padding below title is reduced for divider.
            final int titlePaddingBottom = (int) mBuilder.context.getResources().getDimension(
                    R.dimen.md_title_frame_margin_bottom_list);
            titleFrame.setPadding(titleFrame.getPaddingLeft(), titleFrame.getPaddingTop(), titleFrame.getPaddingRight(),
                                  titlePaddingBottom);
        }
    }
    ...
    protected final void invalidateCustomViewAssociations() {
        ...
        if (mBuilder.customView != null) {
            ...
            if (contentScrollView != null) {
                contentScrollView.setVisibility(
                        mBuilder.content != null && mBuilder.content.toString().trim().length() > 0 ? View.VISIBLE :
                                View.GONE);
            }
            boolean canScroll = titleFrame
                    .getVisibility() == View.VISIBLE && (canListViewScroll() || canContentScroll());
            ...
            if (canScroll) {
                if (content != null) {
                    final int contentVerticalPadding = (int) mBuilder.context.getResources().getDimension(
                ...
                final int titlePaddingBottom = (int) mBuilder.context.getResources().getDimension(
                        R.dimen.md_title_frame_margin_bottom_list);
                titleFrame.setPadding(titleFrame.getPaddingLeft(), titleFrame.getPaddingTop(),
            ...
            if (listType == ListType.MULTI) {
                ...
                if (shouldBeChecked) {
                    ...
                    if (mBuilder.alwaysCallMultiChoiceCallback) {
                        // If the checkbox wasn't previously selected, and the callback returns true, add it to the
                        // states and check it
                ...
                if (mBuilder.autoDismiss && mBuilder.positiveText == null) {
                    // If auto dismiss is enabled, and no action button is visible to approve the selection, dismiss
                    // the dialog
                    dismiss();
    ...
    private boolean sendMultichoiceCallback() {
        ...
        return mBuilder.listCallbackMultiChoice
                .onSelection(this, selectedIndicesList.toArray(new Integer[selectedIndicesList.size()]),
     ...
     *
     * @deprecated Use getActionButton(com.whiteboxteam.gliese.ui.custom.materialdialogs.DialogAction)} instead.
     */
    @Deprecated
    @Override
    ...
    /**
     * Use this to customize any list-specific logic for this dialog (OnItemClickListener, OnLongItemClickListener,
     * etc.)
     *
    ...
    private void invalidateList() {
        ...
        // Hide content
        view.findViewById(R.id.contentScrollView).setVisibility(
                mBuilder.content != null && mBuilder.content.toString().trim().length() > 0 ? View.VISIBLE : View.GONE);
    ...
    public final void setItems(CharSequence[] items) {
        ...
        if (mBuilder.adapter instanceof MaterialDialogAdapter) {
            mBuilder.adapter = new MaterialDialogAdapter(this, ListType.getLayoutForType(listType), R.id.title, items);
        } else {
            throw new IllegalStateException(
    ...
    public static class Builder {
         ...
         *
         * @param medium  The name of font in assets/fonts, minus the extension (null uses device default). E.g.
         *                [your-project]/app/main/assets/fonts/[medium].ttf
         * @param regular The name of font in assets/fonts, minus the extension (null uses device default). E.g.
         ...
         *
         * @param indeterminate If true, an infinite circular spinner is shown. If false, a horizontal progress bar
         *                      is shown that is incremented or set via the built MaterialDialog instance.
         * @param max           When indeterminate is false, the max value the horizontal progress bar can get to.
         * @param showMinMax    For determinate dialogs, the min and max will be displayed to the left (start) of the
         *                      progress bar, e.g. 50/100.
